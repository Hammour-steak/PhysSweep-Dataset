...
def reusable_render_record(
    ...
    three_object = schema in {'physweep_billiards_three_object_scene_v1','physweep_passive_pinball_three_object_scene_v1','physweep_marble_run_three_object_scene_v1'}
    render_record_binds_implementation = (
        two_object or three_object or schema in RENDER_RECORD_IMPLEMENTATION_SCHEMAS
    )
...
def render_source_records(
    ...
    for record in records:
        ...
        if expected_schema == "two_object_specialized":
            if (
                str(metadata.get("schema_version")) not in SPECIALIZED_SCHEMAS
...
def main() -> None:
    ...
    script_root = code_root if args.renderer in {"two_object_specialized", "billiards_three_object", "passive_pinball_three_object", "marble_run_three_object"} else root
