...
from tools.sampling.sample_pybullet_base import manifest_counts as generic_manifest_counts
...
def module_name(root: Path, script: Path) -> str:
    ...
    if relative.suffix != ".py" or relative.name == "__init__.py":
        raise ValueError(f"implementation is not an executable Python module: {script}")
    ...
    *,
    ...
    run(
        [
            sys.executable,
            "-m",
            "tools.sampling.sample_pybullet_base",
            "--root",
            str(root),
            "--bundle",
            str(bundle_path),
        ...
        if slot["generator"] == "billiards":
            run(
                [
                    sys.executable,
                    "-m",
                    module_name(root, implementation_paths["billiards_generator"]),
                    "--root",
                    str(root),
                    "--output",
                    str(scene_dir),
        ...
        if slot["generator"] in passive_fixture_records:
            ...
            run(
                [
                    sys.executable,
                    "-m",
                    module_name(root, implementation_paths[f"{pipeline}_generator"]),
                    "--root",
                    str(root),
                    "--config",
