...
from support_structure import table_structure_boxes
from inclined_support import inclined_plane_geometry
from lighting_quality import floor_glare_guard
...
def apply_camera_relative_shadow_lighting(camera_record: dict[str, Any], lighting_rule: dict[str, Any]) -> dict[str, Any]:
    import bpy  # pylint: disable=import-outside-toplevel
    import mathutils  # pylint: disable=import-outside-toplevel

    rule = lighting_rule.get("shadow_readability_rule", {})
    if not isinstance(rule, dict) or not rule:
        return {"status": "not_requested"}
    camera_position = camera_record.get("position")
    target = camera_record.get("target")
    if not (isinstance(camera_position, list) and len(camera_position) == 3 and isinstance(target, list) and len(target) == 3):
        return {"status": "invalid_camera_record"}
    view_x = float(camera_position[0]) - float(target[0])
    view_y = float(camera_position[1]) - float(target[1])
    view_length = math.hypot(view_x, view_y)
    if view_length <= 1e-6:
        return {"status": "degenerate_camera_xy"}
    view_x /= view_length
    view_y /= view_length
    side = 1.0 if float(rule.get("screen_shadow_side", 1.0)) >= 0.0 else -1.0
    lateral_x = -view_y * side
    lateral_y = view_x * side
    direction_mode = str(rule.get("shadow_direction_mode", "screen_lateral"))
    if direction_mode == "toward_camera_lateral":
        shadow_x = view_x * 0.75 + lateral_x * 0.66
        shadow_y = view_y * 0.75 + lateral_y * 0.66
        shadow_length = math.hypot(shadow_x, shadow_y)
        shadow_x /= shadow_length
        shadow_y /= shadow_length
    else:
        shadow_x = lateral_x
        shadow_y = lateral_y
    key = bpy.data.objects.get("directional_softbox_key")
    fill = bpy.data.objects.get("weak_softbox_fill")
    if key is None or fill is None:
        return {"status": "missing_area_lights"}
    horizontal_distance = float(rule.get("key_horizontal_distance_m", 3.0))
    height_above_target = float(rule.get("key_height_above_target_m", 3.8))
    key.location = (
        float(target[0]) - shadow_x * horizontal_distance,
        float(target[1]) - shadow_y * horizontal_distance,
        float(target[2]) + height_above_target,
    )
    key.rotation_euler = (mathutils.Vector(target) - key.location).to_track_quat("-Z", "Y").to_euler()
    max_fill_ratio = float(rule.get("max_fill_to_key_ratio", 0.14))
    fill.data.energy = min(float(fill.data.energy), float(key.data.energy) * max_fill_ratio)
    transverse_ratio = abs(view_x * shadow_y - view_y * shadow_x)
    minimum = float(rule.get("min_transverse_ratio", 0.72))
    return {
        "status": "passed" if transverse_ratio >= minimum else "failed_transverse_ratio",
        "rule_id": str(rule.get("rule_id", "camera_relative_shadow_readability_v1")),
        "physics_invariant": bool(rule.get("physics_invariant", True)),
        "visual_preflight_required": bool(rule.get("visual_preflight_required", True)),
        "visual_failure_action": str(rule.get("visual_failure_action", "resample_lighting_camera_material_candidate")),
        "evidence_mode": str(rule.get("evidence_mode", "unknown")),
        "shadow_direction_mode": direction_mode,
        "camera_view_xy": [round(view_x, 5), round(view_y, 5)],
        "shadow_direction_xy": [round(shadow_x, 5), round(shadow_y, 5)],
        "transverse_ratio": round(transverse_ratio, 5),
        "minimum_transverse_ratio": round(minimum, 5),
        "key_location": [round(float(value), 5) for value in key.location],
        "key_size_m": round(float(key.data.size), 5),
        "key_energy": round(float(key.data.energy), 5),
        "key_facing_target": True,
        "fill_energy": round(float(fill.data.energy), 5),
    }
