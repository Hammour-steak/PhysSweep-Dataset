...
class PinballVisualContractTests(unittest.TestCase):
    def test_render_worker_passes_data_root_without_mask_arguments(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            script=root/'render_three_object_pinball_scene.py'
    ...
    def test_common_export_keeps_three_materials_fixture_and_motion(self):
        for target in ('P','Q','R'):
            m=fixtures.ThreeObjectPinballMetadataTests().candidate(target);scene=compile_resolved_scene(m,fixtures.ROOT)
            ...
            objects,_=_compact_objects('passive_pinball',m,scene,info,'pinboard',{})
            ...
            fixture=build_fixture_payload('passive_pinball_three_object_v1',m,scene)
            self.assertEqual(len(fixture['physical']['fixture']['colliders']),len(m['physics']['fixture']['colliders']))
            self.assertEqual(_solver_contract('passive_pinball_three_object_v1',m,scene)['solver_iterations'],180)
            self.assertEqual(_compact_semantics(m)['profile'],'pair_control')
        old={'schema_version':'physweep_passive_pinball_scene_v1','semantics':{'dynamic_object_count':1,'profile':'dense_pinfield_descent','motion_profile':'unrelated'}}
        self.assertEqual(_compact_semantics(old)['profile'],'dense_pinfield_descent')

    def test_pinball_three_registry_is_explicit_and_excluded_from_older_counts(self):
        for count in (1,2):
            self.assertNotIn('passive_pinball_three_object',{r['pipeline'] for r in load_specialized_backends(fixtures.ROOT,object_count=count)})
        records=load_specialized_backends(fixtures.ROOT,object_count=3)
        r=next(r for r in records if r['pipeline']=='passive_pinball_three_object')
        self.assertEqual(r['dynamic_object_counts'],[3]);self.assertEqual(r['source_schema_version'],'physweep_passive_pinball_three_object_scene_v1')
